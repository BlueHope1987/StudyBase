...
if __name__ == '__main__':
    tokenizer = AutoTokenizer.from_pretrained(r"StudyBase\helloPython\PyTorch\20250615.LLaMA2\tokenizer_k")
    #修改前"tokenizer_k"提示无法连接huggingface.co读取模型，依赖本地权重目录，依据调试命令行应为基于项目根目录的相对路径，r为原始字符串不转义
    args = ModelConfig(
        dim=1024,
        n_layers=18,
    )
...
#####Attention模块测试
    ...
    print("Output shape:", output.shape) #Output shape: torch.Size([1, 50, 768])
...
#####DecoderLayer模块测试
    ...
    print(out.shape) # 形状和输入的x一样 [batch_size, seq_len, dim] torch.Size([1, 50, 768])

#####Transformer模块测试

    print("-Transformer模块测试")
