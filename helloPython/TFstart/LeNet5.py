...
def variables_lenet5(patch_size = LENET5_PATCH_SIZE, patch_depth1 = LENET5_PATCH_DEPTH_1, 
    ...
    w1 = tf.Variable(tf.compat.v1.truncated_normal([patch_size, patch_size, image_depth, patch_depth1], stddev=0.1))
    b1 = tf.Variable(tf.compat.v1.zeros([patch_depth1]))
 
    w2 = tf.Variable(tf.compat.v1.truncated_normal([patch_size, patch_size, patch_depth1, patch_depth2], stddev=0.1))
    b2 = tf.Variable(tf.compat.v1.constant(1.0, shape=[patch_depth2]))
 
    w3 = tf.Variable(tf.compat.v1.truncated_normal([5*5*patch_depth2, num_hidden1], stddev=0.1))
    b3 = tf.Variable(tf.compat.v1.constant(1.0, shape = [num_hidden1]))
 
    w4 = tf.Variable(tf.compat.v1.truncated_normal([num_hidden1, num_hidden2], stddev=0.1))
    b4 = tf.Variable(tf.compat.v1.constant(1.0, shape = [num_hidden2]))
    
    w5 = tf.Variable(tf.compat.v1.truncated_normal([num_hidden2, num_labels], stddev=0.1))
    b5 = tf.Variable(tf.compat.v1.constant(1.0, shape = [num_labels]))
    variables = {
...
def model_lenet5(data, variables):
    layer1_conv = tf.compat.v1.nn.conv2d(data, variables['w1'], [1, 1, 1, 1], padding='SAME')
    layer1_actv = tf.compat.v1.sigmoid(layer1_conv + variables['b1'])
    layer1_pool = tf.compat.v1.nn.avg_pool(layer1_actv, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')
 
    layer2_conv = tf.compat.v1.nn.conv2d(layer1_pool, variables['w2'], [1, 1, 1, 1], padding='VALID')
    layer2_actv = tf.compat.v1.sigmoid(layer2_conv + variables['b2'])
    layer2_pool = tf.compat.v1.nn.avg_pool(layer2_actv, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')
 
    flat_layer = flatten_tf_array(layer2_pool)
    layer3_fccd = tf.compat.v1.matmul(flat_layer, variables['w3']) + variables['b3']
    layer3_actv = tf.compat.v1.nn.sigmoid(layer3_fccd)
    
    layer4_fccd = tf.compat.v1.matmul(layer3_actv, variables['w4']) + variables['b4']
    layer4_actv = tf.compat.v1.nn.sigmoid(layer4_fccd)
    logits = tf.compat.v1.matmul(layer4_actv, variables['w5']) + variables['b5']
    return logits
