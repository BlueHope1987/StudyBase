...
梯度下降 (gradient descent)神经网络中最常用的优化方法 一种系统的优化方法来更新初始随机的参数 以达到损失函数值不断减小的目的
     ...
     学习率 (learning rate)所有的参数沿着其梯度的反方向移动 距离用学习率α控制
反向传播 (back propagation)梯度下降法为求出函数当前参数点梯度 即损失函数关于神经网络中所有参数的导数 神经网络结构复杂难以得到每个参数导数公式 遂采用反向传播 利用了微积分中导数计算的链式法则(chain rule) 可得到损失函数关于最够一层边权的导数 继续计算关于前一层边权导数 直到得出全部导数 按学习率沿导数反向移动 以使损失值不断下降（有复杂公式 需要参考书本）
