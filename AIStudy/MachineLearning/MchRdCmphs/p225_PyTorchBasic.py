...
丢弃 (Dropout)
2012年由Hinton等人提出 适用于深度学习的解决过拟合的方法
不改变模型的情况下减小模型的复杂度 训练时对于每一批次的数据随机删除网络中一半的隐藏层神经元及其它们相连接的边 包括入边和出边 剩下的网络中计算输出和导数并更新参数 随后恢复所有神经元和连边 下批次训练中 继续随机删除一半的隐藏层神经元
